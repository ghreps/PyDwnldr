...
class Mobile:
    ...
    ARCHIVE = config.get('MOBILE FILES', 'archive')
    PROCESSES = int(config.get('MOBILE FILES', 'processes'))
    ROOT_PATH = config.get('MOBILE FILES', 'path')
    ...
    def run(self, process, queue, lock):
        print('[MOBILE FILES] Process ' + str(process) + ' start')
        sql_rows = set()
        ...
        while True:
            ...
            if not queue.empty():
                ...
                try:
                    ...
                    print('[MOBILE FILES] Cant clear random macs in {0}:\n{1}'.format(
                            file[1], e
                        ))
                    continue
                if self.ARCHIVE:
